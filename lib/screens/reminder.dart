...
//                 });
//                 await NotificationService.cancelReminder();         // Cancel old reminder
//                 await NotificationService.scheduleReminder(minutes);     // Schedule new notification
//                                   // Set alarm in Android clock
//               },
//
//             );
//           },
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            Text(
              isReminderOn ? "🔔 Reminder ON" : "🔕 Reminder OFF",
              style: const TextStyle(fontSize: 22),
            ),
            const SizedBox(height: 20),
            ElevatedButton(
              onPressed: () async {
                await NotificationService.scheduleReminder(intervalMinutes: 1);
                setState(() => isReminderOn = true);
              },
              child: const Text("Start Reminder (1 Min)"),
            ),
            ElevatedButton(
              onPressed: () async {
                await NotificationService.cancelReminder();
                setState(() => isReminderOn = false);
              },
